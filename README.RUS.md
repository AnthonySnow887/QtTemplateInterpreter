...
> ПРИМЕЧАНИЕ:
>
> Если вам требуется, чтобы содержимое блока не выполнилось и осталось в неизменном виде, то следует поместить содержимое блока в кавычки (одинарные ``` ' ``` или двойные ``` " ```):
>
> ```twig
...
Если переменная или атрибут не существует, будет вызвана ошибка интерпретации шаблона с её подробным описанием.
...
- ```[object] escape_special_block (object)``` - экранирование специальных символов блоков управления (```{{ ... }} -> \{\{ ... \}\}; {% ... %} -> \{\% ... \%\}; {# ... #} -> \{\# ... \#\}```). Поддерживаемые типы: QByteArray, QString.
...
Для создания объекта по работе с регулярными выражениями используйте функции:
...
> ПРИМЕЧАНИЕ:
>
> Для создания массива QVariantMap для разделения ключа и его значения можно использовать символ двоеточия ``` : ``` или стрелки вправо ``` => ```.
> Так же эти символы могут одновременно встречаться в одном и том же массиве.
>
> Пример 1:
> ```{"foo": "bar", "abc": 123}```
>
...
> ПРИМЕЧАНИЕ:
>
> Класс QtTIParserArgs имеет возможность разбирать входные функции в качестве аргументов.
> Для этого пользуйтесь конструктором QtTIParserArgs(QtTIParserFunc *parserFunc).
> Если у QtTIParserArgs будет не задан указатель QtTIParserFunc, то разбор функций в качестве аргументов других функций будет игнорироваться!
>
